...
class NixlStorageConfig:
    ...
    def validate_nixl_backend(dynamic_storage: bool, backend: str, device: str):
        if dynamic_storage:  # For now only supports OBJ backend
            ...
            if backend in ("GDS", "GDS_MT", "OBJ"):
                return device == "cpu" or device == "cuda"
            elif backend in ("POSIX", "HF3FS", "DOCA_KV"):
                return device == "cpu"
            else:
                return False
    ...
    def from_cache_engine_config(
        ...
        if config.nixl_buffer_size % align_bytes != 0:
            buffer_size = ((config.nixl_buffer_size + align_bytes - 1) // align_bytes) * align_bytes
            logger.warning(f"Nixl buffer size {config.nixl_buffer_size} is not a multiple of align bytes {align_bytes}, auto aligned to {buffer_size}")
