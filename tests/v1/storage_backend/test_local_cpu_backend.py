...
def _make_backend_cpu_simple(
    ...
    config = create_test_config(cpu_gap_rate=cpu_gap_rate, cpu_gap_count=cpu_gap_count)
    PinMonitor.GetOrCreate(config)
...
class TestBatchedContainsGapsCPU:
    ...
    # ------------------------------------------------------------------
    # Fault injection via cpu_gap_rate
    # ------------------------------------------------------------------

    def test_gap_rate_zero_no_faults(self, memory_allocator):
        backend = _make_backend_cpu_simple(memory_allocator, cpu_gap_rate=0.0)
        keys = [create_test_key(str(i)) for i in range(3)]
        _inject_hot_keys(backend, keys)
        gaps, end = backend.batched_contains_gaps(keys)
        assert gaps == []
    ...
    def test_gap_rate_one_all_hits_suppressed(self, memory_allocator):
        """cpu_gap_rate=1.0 -> every real hit is suppressed; end=0."""
        backend = _make_backend_cpu_simple(memory_allocator, cpu_gap_rate=1.0)
        keys = [create_test_key(str(i)) for i in range(3)]
        _inject_hot_keys(backend, keys)
        gaps, end = backend.batched_contains_gaps(keys)
        assert gaps == []
    ...
    def test_gap_rate_one_real_misses_unaffected(self, memory_allocator):
    ...
    def test_gap_rate_partial_uses_random(self, memory_allocator):
        """cpu_gap_rate=0.5: mock random to control which hits flip."""
        backend = _make_backend_cpu_simple(memory_allocator, cpu_gap_rate=0.5)
        keys = [create_test_key(str(i)) for i in range(4)]
        _inject_hot_keys(backend, keys)

        # 0.3 < 0.5 -> flip (gap), 0.7 >= 0.5 -> keep (hit)
        with patch(
            "lmcache.v1.storage_backend.local_cpu_backend.random.random",
    ...
    def test_gap_rate_not_applied_to_real_misses(self, memory_allocator):
        """random.random is NOT called for absent keys."""
        backend = _make_backend_cpu_simple(memory_allocator, cpu_gap_rate=0.5)
        keys = [create_test_key(str(i)) for i in range(3)]
        _inject_hot_keys(backend, [keys[0]])
    ...
    def test_fault_injected_keys_not_pinned(self, memory_allocator):
        """Keys reported as gaps are never pinned, even if really present."""
        backend = _make_backend_cpu_simple(memory_allocator, cpu_gap_rate=1.0)
        keys = [create_test_key(str(i)) for i in range(2)]
...
class TestBatchedContainsGapsCPURuntimeMutation:
    ...
    def test_gap_rate_mutable_at_runtime(self, memory_allocator):
        """Mutating config.cpu_gap_rate between calls changes injection behavior."""
        backend = _make_backend_cpu_simple(memory_allocator)
        keys = [create_test_key(str(i)) for i in range(4)]
        _inject_hot_keys(backend, keys)
        ...
        backend.config.cpu_gap_rate = 1.0
        gaps, end = backend.batched_contains_gaps(keys)
        assert end == 0

        backend.config.cpu_gap_rate = 0.0
        gaps, end = backend.batched_contains_gaps(keys)
        assert gaps == [] and end == 4

    def test_gap_count_mutable_at_runtime(self, memory_allocator):
        """Mutating config.cpu_gap_count between calls changes injection behavior."""
        backend = _make_backend_cpu_simple(memory_allocator)
        keys = [create_test_key(str(i)) for i in range(6)]
        _inject_hot_keys(backend, keys)

        gaps, end = backend.batched_contains_gaps(keys)
        assert gaps == [] and end == 6

        backend.config.cpu_gap_count = 2
        with patch(
            "lmcache.v1.storage_backend.local_cpu_backend.random.sample",
            return_value=[1, 3],
        ):
            gaps, end = backend.batched_contains_gaps(keys)
        assert gaps == [(1, 2), (3, 4)] and end == 6

        backend.config.cpu_gap_count = 0
        gaps, end = backend.batched_contains_gaps(keys)
        assert gaps == [] and end == 6

    def test_gap_count_threshold_not_met(self, memory_allocator):
        """count mode is skipped when len(keys) < 2 * gap_count."""
        backend = _make_backend_cpu_simple(memory_allocator, cpu_gap_count=3)
        keys = [create_test_key(str(i)) for i in range(5)]
        _inject_hot_keys(backend, keys)
        gaps, end = backend.batched_contains_gaps(keys)
        assert gaps == [] and end == 5

    def test_gap_count_takes_precedence_over_rate(self, memory_allocator):
        """When both count and rate are non-zero, count mode wins."""
        backend = _make_backend_cpu_simple(
            memory_allocator, cpu_gap_rate=1.0, cpu_gap_count=2
        )
        keys = [create_test_key(str(i)) for i in range(6)]
        _inject_hot_keys(backend, keys)
        with patch(
